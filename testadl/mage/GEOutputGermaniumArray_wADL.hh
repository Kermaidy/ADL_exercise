 ...
 *
 *   03-30-2017, Y. Kermaidic, Added ADL trace simulation output for each hits
 *
 */
...
// ADL includes
#include "adl/GEADLDetectorTrace.hh"
...
#include "gerdaio/GEOutputGermaniumArray_wADLMessenger.hh"
...
class GEOutputGermaniumArray_wADL : public MGOutputRoot
{
public:
  //default constructor
  GEOutputGermaniumArray_wADL(G4bool isMother, G4bool needt, G4bool larInst = false);

  //copy constructor
  GEOutputGermaniumArray_wADL(const GEOutputGermaniumArray_wADL &);
...
//{MGLog(routine)<<"now processing tracks in waiting stack"<<endlog;};
  ...
  void SetSimulateTraces(G4bool bb){fSimulateTraces = bb;}
  G4bool GetSimulateTraces(){return fSimulateTraces;}

  void SetRecordADLTraces(G4bool bb){fRecordADLTraces = bb;}
  G4bool GetRecordADLTraces(){return fRecordADLTraces;}

  void SetRecordADLOutPos(G4bool bb){fRecordADLOutPos = bb;}
  G4bool GetRecordADLOutPos(){return fRecordADLOutPos;}
...
//  void SimulatePulse(int,double,double,double); // ADL-4.1
  void SimulatePulse(int,int,double,double,double);   // ADL-4.2

  static const G4int MAX_NTRACE=2000;
  ...
  G4int fArrayDetectorTypeMatrix[7][7][2];
  ...
  G4bool fSimulateTraces;
  G4bool fRecordADLOutPos;
  G4bool fRecordADLTraces;
  ...
  GEOutputGermaniumArray_wADLMessenger* fMessenger;
  ...
  int isAlreadyRead[40];   // Flag to read only once E/W/Stru potential from files (then stored into memory)
  int traceCalculated[40]; // Flag to calculate trace in Ge det only once per event 
