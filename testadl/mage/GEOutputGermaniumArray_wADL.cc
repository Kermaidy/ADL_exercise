...
//---------------------------------------------------------------------------//
//                                                          
// $Id: GEOutputGermaniumArray_wADL.cc,v 1.34 2009-03-25 15:16:55 schubert Exp $ 
//      
// CLASS IMPLEMENTATION:  GEOutputGermaniumArray_wADL.cc
//
//---------------------------------------------------------------------------//
/**
 * SPECIAL NOTES:
...
#include "gerdaio/GEOutputGermaniumArray_wADL.hh"    
...
//---------------------------------------------------------------------------//
GEOutputGermaniumArray_wADL::GEOutputGermaniumArray_wADL(G4bool isMother, G4bool needt, G4bool larInst):
      ...
      fAutoSaveSize(10000000)
{
  ...
  fMessenger = new GEOutputGermaniumArray_wADLMessenger(this);
...
void GEOutputGermaniumArray_wADL::DefineSchema()
{
  if(!SchemaDefined()){
    ...
    if(fSimulateTraces){
      for(G4int i=0; i < 40; i++) isAlreadyRead[i] = 0;
      setupADLdetPos = 0;
      ...
      //ADLDetector1 = new GEADLDetectorTrace(1);
      //ADLDetector2 = new GEADLDetectorTrace(2);
...
void GEOutputGermaniumArray_wADL::BeginOfRunAction()
{
  if (!fRunInitialized)
  {
...
void GEOutputGermaniumArray_wADL::BeginOfEventAction(const G4Event *event)
{
...
void GEOutputGermaniumArray_wADL::FillMCToNtupleTrees(const G4Event *event)
{
...
void GEOutputGermaniumArray_wADL::EndOfEventAction(const G4Event *evt)  
{
...
void GEOutputGermaniumArray_wADL::FillMotherNucleiToNtupleTrees(const G4Event *event)
{
...
void GEOutputGermaniumArray_wADL::FillHitsToNtupleTrees(const G4Event *evt){
  //
  // hits in sensitive detectors
  //
  ...
  if (GCHC){
    ...
    for (G4int i=0; i<hits_totnum; i++) {
      ...
      while(!fCrystalFound){
        fOuterCounter++;
        fInnerCounter = -1;
        while(!fCrystalFound && fInnerCounter < MaxNrDetInOneString-1){
          fInnerCounter++;
            ...
            else if(theDetDB->IsAPair(fOuterCounter,fInnerCounter))
            {
            	fCopyNumberCounter++;
                if(fCopyNumberCounter == (*GCHC)[i]->GetCopynumber() ){
	...
	zDet[hits_iddet[i]] = theDetDB->GetCrystalPosition(fOuterCounter,fInnerCounter).z();
      ...
      /*
	std::cout << "Detector position : " << i 
	<< " " << detPar->GetGeometry() 
	<< " " << hits_iddet[i] 
	<< " " << detPar->GetDetectorID() 
	<< " " << fOuterCounter
	<< " " << fInnerCounter
	<< " " << theDetDB->GetCrystalPosition(fOuterCounter,fInnerCounter).x()
	<< " " << theDetDB->GetCrystalPosition(fOuterCounter,fInnerCounter).y() 
	<< " " << theDetDB->GetCrystalPosition(fOuterCounter,fInnerCounter).z()
	<< std::endl;
      */
    ...
    for (G4int i=0; i<hits_totnum; i++) {
      if(fSimulateTraces && hits_iddet[i] > 2 && traceCalculated[hits_iddet[i]] == 0){
...
void GEOutputGermaniumArray_wADL::FillTrajectoriesToNtupleTrees(const G4Event *evt)
{
  //
  // all trajectories
  //
...
void GEOutputGermaniumArray_wADL::FillNtuple()
{
...
void GEOutputGermaniumArray_wADL::WritePartialEvent(const G4Event* evt)
{
  this->FillHitsToNtupleTrees(evt);
  if (NeedTrajectoryInformation){
    this->FillTrajectoriesToNtupleTrees(evt);
...
// void GEOutputGermaniumArray_wADL::PrepareNewEvent(const G4Event* /*evt*/)
// {
//   //if (NeedTrajectoryInformation){
//   //  this->FillTrajectoriesToNtupleTrees(evt);
//   //}
...
void GEOutputGermaniumArray_wADL::ResetPartialEvent(const G4Event* evt)
{
...
void GEOutputGermaniumArray_wADL::EndOfRunAction()
{
...
void GEOutputGermaniumArray_wADL::RootSteppingAction(const G4Step *step)
{
...
G4ClassificationOfNewTrack GEOutputGermaniumArray_wADL::StackingAction(const G4Track *track)
{
...
//------------------------------------------------------------------------------------------
void GEOutputGermaniumArray_wADL::PreUserTrackingAction(const G4Track* aTrack)
{
  if (NeedTrajectoryInformation || MGVOutputManager::GetUseTimeWindow())
  {
...
//------------------------------------------------------------------------------------------
void GEOutputGermaniumArray_wADL::FillDetectorTypeMatrix()
{
  MGGeometryDetector* theDet = 
      MGManager::GetMGManager()->GetManagerDetectorConstruction()->GetDetector();
  ...
  for (G4int j = 0; j < MaxNrDetInOneString; j++)
    for (G4int i = 0; i < theDetDB->GetDN500NStrings(); i++)
      for (G4int k = 0; k < 2; k++)
	fArrayDetectorTypeMatrix[i][j][k] = theDetDB->GetCrystalType(i,j,k);

}
//------------------------------------------------------------------------------------------
void GEOutputGermaniumArray_wADL::NewStage(){
...
void GEOutputGermaniumArray_wADL::StoreLArInstPartMap() {
...
void GEOutputGermaniumArray_wADL::SimulatePulse(int isalreadyread,int channel, double x,double y,double z){
  ...
  //  GEADLDetectorTrace* ADLDetector = new  GEADLDetectorTrace(debugADL,channel);
  
  ADLDetector->SetSetupFile(channel);
  //  ADLDetector->ConfigureADL(ADLDetector->GetSetupFile()); // ADL-4.1
  ...
  if(hits_tote > 0.5){
    ...
    MGTree->Fill();
    if(debugADL) MGLog(trace) << "Print MGTree : " << MGTree->GetEntries() << " events recorded" << endlog;
  }
